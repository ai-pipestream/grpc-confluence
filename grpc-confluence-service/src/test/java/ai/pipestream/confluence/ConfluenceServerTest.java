...
/**
 * {@link ConfluenceServer#startNetty} end to end: a real Netty socket,
 * the health service reporting SERVING once listening, and reflection
 * serving the facade's descriptor.
 */
class ConfluenceServerTest {
    ...
    private void startOnEphemeralPort() throws Exception {
        server = ConfluenceServer.startNetty(
    ...
    void reflectionServesTheFacadeDescriptorOverTcp() throws Exception {
        ...
        requests.onNext(ServerReflectionRequest.newBuilder()
                .setFileContainingSymbol("ai.pipestream.confluence.v1.ConfluenceService")
                .build());
        ...
        assertThat(files).contains("ai/pipestream/confluence/v1/confluence_service.proto");
    }
}
